...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username.toLowerCase() === 'admin' && password === 'admin') {
      ...
      // Gecikme hissi vermek için kısa bir timeout
      setTimeout(() => {
    ...
    try {
      ...
      if (!response.ok) {
        // Fallback: API hatası durumunda da girsin istendiği için (önceki requestlerde)
        const mockData = {
          access_token: "mock_token_" + Date.now(),
          user: {
            full_name: username || "Hakan Buğra Erentuğ",
            username: username || "hakan",
            employee_id: "EMP-001"
          }
        };
      ...
      // Bağlantı hatası olsa bile girişe izin ver
      const mockData = {
        access_token: "mock_token_" + Date.now(),
        user: {
          full_name: username || "Hakan Buğra Erentuğ",
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Sicil no veya isim..."
              />
            </div>
